# qiankun JS 沙箱:拿进来的代码在什么环境里跑(第 3 篇)

> [← 返回总纲](./README.md) · 本系列第 3 篇:为什么需要沙箱 / 三代实现演进 / Proxy 沙箱原理 / 边界:它挡不住什么

第 2 篇说脚本被"带进沙箱执行"——沙箱就是本系列最重要的隔离墙一。**微前端最难的从来不是"把多个应用拼在一起",而是"拼在一起后,它们彼此的全局变量不要互相污染"。** 这一篇先讲清墙为什么要建,再拆解 qiankun 的沙箱实现与边界。

## 一、为什么必须有 JS 沙箱

浏览器里同一时刻只有一个 `window`、一个全局作用域。子应用跑起来会不自觉地做这些事:

- 声明全局变量、往 `window.xxx` 挂东西(工具库、SDK、polyfill、缓存);
- 改全局对象(如给 `Array.prototype` 打补丁);
- 挂全局监听(`window.addEventListener`)与定时器。

如果 A、B 两个子应用共享真 `window`:
...
qiankun 的思路是:**给每个子应用一个"假的 window"去玩**,尽量别让脏手碰到真 `window`。注意它的目标是"全局变量隔离",**不是** iframe 那种"连 DOM、事件、会话全隔离"(那属于[生态篇](./ecosystem.md)里无界等 iframe 方案的取舍)。

## 二、三代演进:从"拍快照还原"到"Proxy 隔离"

qiankun 沙箱按实现思路走过了三代,读源码/面试时按这条线理解最顺:
...
2. **Proxy 把"还原"变成了"不用脏"**:既然每次写都只落在子应用自己的假 window 上,激活/失活就只剩一个开关,多实例共存成为可能。

> 现役实现选择:**环境支持 Proxy → ProxySandbox(默认);不支持(如旧 IE)→ SnapshotSandbox 降级**。这也是为什么 qiankun 文档里"多实例同存"总伴随"需 Proxy 沙箱"。

## 三、ProxySandbox 怎么工作:读回退、写私有

核心思想一句话:**读——自己有的读自己的,自己没有的回退真 window;写——只写进自己的私有上下文,绝不落真 window。**
...
qiankun 的真实实现远比上面复杂,你读源码时会看到它在处理这些"脏活":

- **`window`、`self`、`globalThis` 三个名字全指向 proxy**,堵住"换名字绕过"的路;
- **方法要绑对 `this`**:真 window 上很多方法(如 `alert`、`addEventListener`)被单独取出来调用时会丢 `this`,qiankun 要按需绑定/特判;
- **不可配置 / 危险属性要挡**:`location` 等不能真让你在假 window 里改着玩,有专门白名单/黑名单;
- **Symbol 陷阱**:`Symbol.unscopables`、`Symbol.toStringTag` 等要返回正确形状,否则框架代码会莫名崩。

**效果**:子应用 A 里 `window.__x = 1`,只落在 A 的假 window;切到 B 读 `window.__x` → 没有 → 回退真 window(也没有)→ `undefined`。真 window 全程没被碰过,所以"激活/失活"真的只剩 `running` 开关。

## 四、它的边界:能挡什么,挡不住什么(最重要的一节)

读透这张表,你对沙箱的理解就超过大多数人:
...
1. **沙箱管 JS 变量,不管 DOM 与事件**:unmount 要对称清理 DOM、监听、定时器(与[模型篇](./model.md)的"mount/unmount 对称"呼应);
...
- 关于 Vite 子应用在沙箱里"跑了但沙箱失效"的特殊情况,见[工程实战篇](./migration.md)。
