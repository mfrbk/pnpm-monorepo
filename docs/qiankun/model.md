...
> 本系列第 1 篇:为什么微前端要"生命周期化" / 注册与启动 / activeRule 路由活动 / loadMicroApp 手动加载。· [← 返回总纲](./README.md)

qiankun 能让一个"独立网页应用"被装进另一个应用、切走、再装回来,靠的是:子应用不再是"自己启动的页面",而是**一个暴露了若干生命周期函数的模块**,由主应用决定何时调用哪个函数。

## 一、核心思想:把应用从"启动"改成"被调度"

普通单页应用**自己启动**:入口 JS 被加载,立刻创建根组件、mount 到 `#root`,生命周期由框架内部管理。

微前端要求子应用**让出启动权**:把"何时激活、挂到哪"交给主应用。于是子应用入口从"一段启动代码"变成**一个带 `bootstrap / mount / unmount` 的模块**(single-spa 的 lifecycle 抽象,qiankun 完整继承):
...
iframe 的隔离"强得过头"(连 DOM、会话都分开,后续沟通成本极高);single-spa 式"软集成"共享同一环境,**代价是需要沙箱与样式隔离来补墙**(见[沙箱篇](./sandbox.md)、[样式篇](./style-isolation.md))。
...
- **同一份代码、两种环境**:靠 `window.__POWERED_BY_QIANKUN__` 判断"独立跑"还是"被挂载"——开发时子应用独立运行(有 HMR),上线后被 qiankun 加载。
- **`props` 是主应用与子应用的唯一握手通道**:固定包含 `name`、`container`(当前挂载容器,供渲染定位);启用全局状态时还注入通信方法(见[通信篇](./communication.md));其余是主应用注册时透传的业务数据。
- **`instance` 要挂在模块外、每次 mount 重建**:保证"反复进入 = 反复挂载/卸载",而不是重复创建导致内存泄漏或渲染叠加。

## 三、注册与启动:主应用说清"谁 + 何时 + 挂哪"
...
> 主应用一般要加错误兜底:`addErrorHandler((err) => ...)`(single-spa 的错误机制)统一捕获子应用加载失败、生命周期抛错并上报。**规则是先 register 后 start**,注册一般在路由就绪前完成,避免首屏跳变。

## 四、activeRule:路由如何"叫醒"子应用

`activeRule` 是子应用的"闹钟",支持三种写法,命中时自动 `mount`,离开时自动 `unmount`:
...
1. **主应用路由与子应用路由"各管一段"**:通常约定主应用用自己的路由管"框架页",`activeRule` 只认某个前缀(如 `/react`),该前缀**以下的路径交给子应用内部路由**(react-router 设 `basename`、vue-router 设 `base`,均取该前缀)。
2. **历史路由模式决定刷新是否 404**:子应用用 history 模式时,直接刷新 `/react/foo` 需要服务器把该前缀回退到子应用入口 HTML(部署时要配),否则 404——微前端部署最常见的坑之一。

## 五、手动加载:loadMicroApp 与"多实例 / 嵌入式"

`registerMicroApps` 是**路由驱动**的整页应用;**手动加载** `loadMicroApp` 适合"页面内嵌一块子应用"或"同页同时存在多个实例":

```js
import { loadMicroApp } from 'qiankun'

// 返回该微应用的控制器实例(加载完成即自动挂载;可用实例方法手动控制)
...
> 单实例 vs 多实例是理解 qiankun 沙箱演进的钥匙:快照沙箱只能"一次一个",Proxy 沙箱才是"各玩各的"(见[沙箱篇](./sandbox.md))。
