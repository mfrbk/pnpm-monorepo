...
> 本系列第 4 篇:何时选 Rollup / Vite 为何以它做生产引擎 / 生态源码阅读路线图。· [← 返回总纲](./README.md)

三兄弟都叫"打包器",但**服务对象不同、价值曲线不同**。

## 一、Rollup vs Webpack:判断依据就一条

> **目标产物是不是"给其他代码 import 的模块"?是 → Rollup;是"给用户打开的可部署应用" → Webpack/Vite。**

类库的消费者是**下一层代码**;应用的消费者是**浏览器里的用户**:
...
- **React** 官方产物(ES/CJS/UMD 多格式)同样由 Rollup 产出——"一把源码出多格式"是它的看家本领;
- **lodash-es** 代表"以 ESM 形态发布、把摇树权交给下游"的库形态(理念与 [核心篇](./core.md) 的"保留 export"同源)。

**什么时候回头用 Webpack/Vite**:目标是"完整 Web 应用",需要 HMR、页面资源、代码分片、MF 等整套应用管线时——强行用 Rollup 搭应用等于把内置的活全手动拼装。应用侧对比见 [Webpack 工程化篇](../webpack/engineering.md) 与 [Vite 生态篇](../vite/ecosystem.md)。
...
**Vite 的 `build` 命令,底层就是 Rollup。**

回顾 [Vite 双引擎](../vite/architecture.md):开发期 Vite 靠"原生 ESM + esbuild 预构建"做到毫秒级,但"不打包"只服务开发;上线前仍要打包(请求数、摇树、分割、压缩),这一步交给 **Rollup**——Vite 复用 Rollup 的打包能力与插件生态,自己补上 dev server / 预构建 / 资源管线那层。

**这个关系带来三个"于是"**:

1. **Vite 配置里出现 `build.rollupOptions` 不是巧合**:Vite 把 Rollup 选项原样透传,[配置篇](./config.md) 学的 `input/external/output` 心智在 Vite 里直接可用;
2. **Rollup 插件能用在 Vite 构建**:Vite 插件是"Rollup 插件接口 + 若干 Vite 专属钩子"的超集(见 [Vite 插件篇](../vite/plugins.md));
3. **未来 Rolldown 也是平滑的**:Vite 正用 Rust 的 Rolldown 逐步替换生产引擎以统一 dev/build,仍是"Rollup 式心智"——**学透 Rollup,等于提前投资了 Vite 的现在与未来**。
...
Rollup 官方插件普遍**小而克制**(常在几百行内),且遵循"极简内核 + 单职责钩子"哲学,**是读码学习的绝佳样本**:
...
1. **先找钩子,再看业务**:钩子函数(`resolveId/load/transform/...`)就是骨架,顺着钩子读实现最快;
2. **留意 `return null` 与 `\0`**:`null` = 交还控制权,`\0` = 内部虚拟 id——两处约定藏着插件协作的礼仪;
3. **对照钩子表读**:把 [插件篇](./plugin-api.md) 的生命周期图摆在旁边,读到哪个钩子就去图里找位置。

读毕再看 Vite 构建管线([插件篇](../vite/plugins.md) + Vite 源码 `packages/vite/src/node/build.ts`),自然打通"Rollup 能力层"与"Vite 应用层"。
...
> 本仓库 `@mzy1120/*` 子包(如 `@mzy1120/http`)选用 **tsup**(esbuild 打码 + Rollup 合并 d.ts)走同一条"库工程"路径:esm + cjs + d.ts、`files` 仅 `dist`、`sideEffects: false`。想换"裸 Rollup"手控细节,把 [配置篇](./config.md) 的样板搬过去即可。
