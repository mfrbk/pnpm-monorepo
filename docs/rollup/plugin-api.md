...
> 本系列第 3 篇:钩子生命周期 / 手写自定义插件 / rollup() 与 watch() 编程化构建。· [← 返回总纲](./README.md)

Rollup 的插件模型很简单——**导出一个带 `name` 与若干钩子函数的对象**。要用对钩子,先要知道它们**在构建的哪个阶段、以什么顺序触发**。

## 一、钩子生命周期:构建钩子 vs 输出钩子

一次构建分两个大阶段,插件钩子也据此分两组:
...
- 钩子可同步也可返回 Promise(异步);`resolveId/load/transform` 对**每个模块**都跑,`generateBundle/writeBundle` 每次构建跑一次。
  ...
  return {
    ...
    transform(code) {
      ...
      return {
        ...
        map: null, // 没改结构,可不提供 map;改行号时需传 map
      }
    },
  }
}
...
源码里写 `export const VERSION = '__VERSION__'`,构建时注入真实版本号。**`transform` 返回 `null` = 不处理**,是插件的默认姿势。

### 样本 B:虚拟模块 + 输出额外产物

虚拟模块 = "不存在的文件,由插件凭空造出来",可向业务代码注入构建期数据:
...
业务代码 `import { builtAt } from 'virtual:build-meta'` 即可。示范三件事:**`resolveId` 拦截导入、`load` 凭空造模块、`generateBundle` + `this.emitFile` 往产物追加文件**。

### 样本 C:自动注入版本头(banner)

更省事的是 Rollup **内建的 `banner` 输出选项**(给每个 chunk 头部加注释),插件只在需要**复杂逻辑**时才出手:
...
命令行 `rollup -c` 只是 JS API 的一层薄壳。用 `rollup()` / `watch()` 编程,可实现"构建 + 加工 + 发布"的自动流水线。
...
// ③ 类型由 tsc 单独产出(声明与打包分离,同 Vite 的"转译/检查分离")
...
用脚本管理"监听重构建",用 `watch()` 返回的 watcher + 事件订阅:
...
**分层**:先找官方插件,再考虑小钩子(transform/banner),最后才是完整插件或 JS API——**Rollup 的哲学是"内核极简 + 外挂补齐",插件应一事一职、返回 `null` 保持谦逊**。
...
> 官方文档:rollupjs.org/plugin-development、rollupjs.org/javascript-api;官方插件源码见 github.com/rollup/plugins。下一篇:[对比 Webpack/Vite 与生态视野](./ecosystem.md)。
