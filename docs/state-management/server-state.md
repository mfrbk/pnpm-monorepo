...
## 二、症状学:手写 store 缓存常见的五宗罪

用通用 store 管理请求数据,典型问题:
...
**分界原则一句话**:服务端数据进 Query 缓存;**客户端交互态**进 store。交互态里属于"某个具体组件"的(草稿、开关)连 store 都别进:
...
| 主题              | 一句话记住                                                                                                   |
| ----------------- | ------------------------------------------------------------------------------------------------------------ |
| Server State 本质 | 远端真源的本地缓存:真源在远端、随时被别人改、变更天然异步、看到的永远是某刻快照                              |
| 塞进 store 的代价 | 手搓三态 / 失效 / 去重、无过期概念、乱序覆盖——一套烂缓存                                                     |
| Query 心智        | queryKey 是缓存地址;同 key 数据共享、只发一次请求;**status 与 fetchStatus 解耦**(后台刷新旧数据顶着、不闪屏) |
| 两把时间锁        | **staleTime** 管"多信任缓存多久"(默认 0,过期触发后台刷新);**gcTime** 管"没人看还能留多久"(约 5 分钟)         |
| 写路径            | 读用 `useQuery`、写用 `useMutation`;乐观更新 = cancel → setQueryData → 失败回滚 / 成功 invalidate            |
