...
> 本系列第 4 篇:何时选 Vite 何时留 Webpack / 框架与元框架版图 / Vitest 与 Storybook 工具链。· [← 返回总纲](./README.md)

把 Vite 放回整个前端工程化坐标系:**快是有代价的,选型按项目诉求来**。
...
**一个神话要破除**:"Vite 生产更快更小"并不总是成立。双引擎下,dev 的"快"来自**不打包**,而这套优势**不延伸到 build**(build 仍是 Rollup 全量打包,甚至比 Webpack 大项目快不了多少)。**把 Vite 的"快"精确理解为"开发快"**。

### 什么场景首选 Vite

- **新起的 SPA / 中后台**:开发体验是压倒性优势;
- **追求极致的开发反馈**(改样式、改组件要毫秒级);
- **框架官方已把 Vite 当默认**(Vue、Svelte、Solid 等),跟着生态走最省心。

### 什么场景仍要 Webpack

- **历史大型项目,重度依赖 Webpack 专属插件/配置**:迁移成本远大于收益;
- **需要 Webpack 特有能力**:某些老牌插件的独家能力、极度细粒度的 loader 定制;
- **团队已沉淀大量 Webpack 经验与基建**:"够用 + 熟练"比"更快"更重要;
- 微前端里 **Module Federation 的本家实现**在 Webpack(Vite 侧用社区插件,成熟度看项目取舍)。

> 判断标准:**新项目、Dev 体验优先 → Vite;老项目、重度定制与既有生态锁定 → 留在 Webpack**。Webpack 侧的完整原理可对照 [Webpack 系列](../webpack/README.md)。

## 二、框架支持:不是"Vue 专属"

Vite 核心**框架无关**:核心只做模块与资源,Vue/React/Svelte 的编译全部由**插件**接入,对任何框架接入成本都是一个插件。
...
每个主流框架都长出了自己的元框架,底层都是 Vite。**学好 Vite 本身,等于同时理解 Nuxt / SvelteKit / Astro 这些框架的构建底层**——技能可跨框架迁移。

## 三、测试与周边生态:一套配置通吃

### Vitest

Vitest 直接把 Vite 当底层,alias、plugins、env、tsconfig 处理在测试环境原样生效,不必像 Jest 时代单独维护一套 transform 配置:
...
Storybook 原生支持 Vite(builder),组件库的**开发/文档/测试/示例**与真实应用共享同一套构建规则。
...
**少维护 N 套 build 配置**,是 Vite 生态在"纯性能"之外常被低估的生产力优势。
...
- **Vite 的"快"是开发快**,build 环节两家处于同一量级;评估分"开发体验"与"产物/构建"两本账;
- **跟着生态走**:框架官方默认用谁,新项目就优先信谁;
...
> 官方文档:vite.dev/guide(生态与框架)、vitest.dev、storybook.js.org。整个系列回到 [总纲](./README.md);对照阅读:[Webpack 学习系列](../webpack/README.md)。
