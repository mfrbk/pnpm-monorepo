...
> 本系列第 2 篇:vite.config 全解 / `.env` 与 `import.meta.env` / `index.html` 即入口。· [← 返回总纲](./README.md)
...
`defineConfig` 提供类型提示与校验,拼错字段立刻报错。

## 二、高频配置项

### base:子路径 / CDN 部署时的资源路径

SPA 默认 `base: '/'`,所有资源 URL 从根开始拼,部署在子路径(如 GitHub Pages 的 `/my-repo/`、统一网关下的某段路由)或 CDN 时必 404:
...
配套地,Vite 暴露 `import.meta.env.BASE_URL`(默认 `/`),做需要感知部署路径的运行时拼接(如动态加载的图片):
...
深层组件引公共工具,`import x from '../../../utils'` 既丑又脆(一挪目录全断):
...
> Vite **默认不会**替你读 `tsconfig` 的 `paths` 映射(除非装 `vite-tsconfig-paths` 之类插件),别名靠 `resolve.alias` 显式声明;用 `fileURLToPath(new URL(...))` 而非 `__dirname`,因为 `vite.config.ts` 默认按 ESM 处理。
...
普通样式 `import './index.css'` 走全局;需要隔离的写 `X.module.css`,导入后得到作用域化的类名对象:
...
开发时前端在 `localhost:5173`、后端接口在 `localhost:8080`,直接 `fetch('/api/...')` 必然跨域。由 dev server 做一层代理,前端永远只写相对路径:
...
**proxy 只活在 dev**:产物上线后由网关 / Nginx 承担同样的转发职责。代码里永远用相对路径 `/api/...`,把"后端到底在哪"留给环境层——这是前后端分离的黄金习惯。
...
后读的覆盖先读的,`.local` 系优先级最高——放个人本地的调试地址,不该提交进仓库(记得进 `.gitignore`)。
...
代码里通过 `import.meta.env` 访问,并带上几个内建字段:
...
**为什么强制 `VITE_` 前缀?这是安全设计。** 客户端能拿到的 env 会**直接打进产物**,任何人都能在浏览器里看到。默认 `envPrefix: 'VITE_'`,只有带这个前缀的变量才会被注入到 `import.meta.env`——`package.json` 里的 `NPM_TOKEN`、`SECRET_KEY` 等敏感项永远不会泄漏进前端 bundle:
...
> 铁律:**凡是 `VITE_` 开头的,都等于公开**。任何密钥、Token 一律不要用 `VITE_` 暴露;要读真正的服务端机密,走代理/网关。

### env 类型补全

`.env` 的键 Vite 不熟,`import.meta.env.VITE_XXX` 默认是 `any`/`string`。建 `src/vite-env.d.ts`:
...
Webpack 的入口藏在 `webpack.config.js` 的 `entry` 里;Vite 把它明晃晃放在项目根目录的 `index.html`:
...
- 好处:**入口即所见**——打开 HTML 就能知道应用从哪启动,多页应用只需多放几个 html 并声明入口。

**多页应用(MPA)**:多个 html 各带自己的 module 入口,构建时声明为输入:
...
**与 `public/` 的分工**:`public/` 里的文件**不做任何处理**(不转译、不哈希、不可 import),原样拷进产物根,通过绝对路径 `/xxx.png` 引用——适合 favicon、robots.txt、第三方不经过构建的脚本;`src/assets/` 下**经过 import 的资源**才走打包管线(哈希、压缩、懒加载)。判据:**要处理的走 import,不处理的放 public**。
